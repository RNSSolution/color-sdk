...
The [Cosmos-SDK](https://github.com/ColorPlatform/color-sdk) is a framework for building multi-asset public Proof-of-Stake (PoS) blockchains, like the Cosmos Hub, as well as permissionned Proof-Of-Authority (PoA) blockchains.
...
Tendermint passes transactions from the network to the application through an interface called the [ABCI](https://github.com/ColorPlatform/prism/tree/master/abci), which the application must implement. 
