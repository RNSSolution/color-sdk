...
func makeMsg(name string, pk crypto.PubKey) auth.StdTx {
	...
	msg := staking.NewMsgCreateValidator(sdk.ValAddress(pk.Address()), pk, sdk.NewInt64Coin(defaultBondDenom,
		50), desc, comm, sdk.OneInt(), sdk.OneInt(), sdk.OneInt())
...
func TestGaiaGenesisValidation(t *testing.T) {
	...
	genesisState = makeGenesisState(t, genTxs)
	nodeID := sdk.NewInt(int64(0))
	leagueID := sdk.NewInt(int64(0))
	val1 := staking.NewValidator(addr1, pk1, staking.NewDescription("test #2", "", "", ""), nodeID, leagueID)
	val1.Jailed = true
	...
	val1.Jailed = false
	genesisState = makeGenesisState(t, genTxs)
	val2 := staking.NewValidator(addr1, pk1, staking.NewDescription("test #3", "", "", ""), nodeID, leagueID)
	genesisState.StakingData.Validators = append(genesisState.StakingData.Validators, val1)
